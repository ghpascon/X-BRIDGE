...
class ReaderHelpers:
    async def configure_interface(self, session):
        return await self.post_to_reader(session, self.endpoint_interface, payload=self.interface_config, method="put")
    ...
    async def start_inventory(self, session=None):
        ...
        return await self.post_to_reader(session, self.endpoint_start, payload=self.config.get("READING_CONFIG"))    
