...
class SetupReader:
    ...
    async def check_error_response(self):
        ...
        self.current_timeout_answer = current_ms if not self.wait_answer else self.current_timeout_answer
    ...
    async def setup_inventory_mode(self):
        await self.send_data([0xa5, 0x5a, 0x00, 0x0c, 0x70, 0x01, 0x01, 0x00, 0x00, 0x00, 0x0d, 0x0a])
    ...
    async def setup_antennas(self):
        ...
        if antenna is not None:
            for i in range(self.ant_qtd):
                ...
                if ant_n is None:continue
                active = ant_n.get("active")
                if active is None or not active:continue
                b_antenas |= 0x1 << (i)
        if b_antenas == 0:
            b_antenas = 1

        await self.send_data([0xa5, 0x5a, 0x00, 0x0d, 0x28, 0x01, 0x00, b_antenas, 0x00, 0x00, 0x00, 0x0d, 0x0a])
    ...
    async def setup_fastid_3(self):
        await self.send_data([0xa5, 0x5a, 0x00, 0x0c, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0d, 0x0a])
    ...
    async def setup_buzzer(self):
        ...
        await self.send_data([0xa5, 0x5a, 0x00, 0x0a, 0xa1, 0x07, 1 if buzzer else 0, 0x00, 0x0d, 0x0a])
    ...
    async def setup_power_ant(self, ant=1):
        try:
            ...
            low = value & 0xFF          
            print("send")
            await self.send_data([0xa5, 0x5a, 0x00, 0x0e, 0x10, 0x02, ant, high, low, high, low, 0x00, 0x0d, 0x0a])
